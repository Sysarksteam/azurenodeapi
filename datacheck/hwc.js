...
function checkhwcusercase(res) {
    logger.trace("Total Records to Sync - " + res.length);
        ...
        con_mdb.query(hwc_insert_dupQuery, inserthwc_dupdataset, function (error, dup_result, fields) {
            if (error) {
                ...
                if (dup_result.affectedRows > 0){
                    logger.trace("HWC Flagged Record - " + JSON.stringify(dup_result.affectedRows) + " :: " + counter);
        ...
        con_mdb.query(hwc_insertQuery, setHWCdata(ucdata), function (error, uc_result, fields) {
            if (error) {
                ...
                if (uc_result.affectedRows > 0) {
                    logger.trace("HWC Record inserted : " + JSON.stringify(uc_result.affectedRows) + " :: " + dataInserted);
    ...
    dbconn.mdb.then(function (con_mdb) {
        con_mdb.query(hwc_insertFlaggedQuery, setHWCdata(ucdata), function (error, uc_result, fields) {
            if (error) {
                ...
                insert_hwc_livestock(ucdata);
                if (uc_result.affectedRows > 0) {
                    // logger.trace("Flagged HWC Record inserted : " + JSON.stringify(uc_result.affectedRows) + " :: " + dataFlagged);
                    console.log("Flagged HWC Record inserted : " + JSON.stringify(uc_result.affectedRows) + " :: " + dataFlagged++);
                }
            }
        });
    }).catch(err => {
                ...
                con_mdb.query(hwc_crop_insertQuery, inc_data, function (error, crop_result, fields) {
                    if (error) {
                        ...
                        if (crop_result.affectedRows > 0)
                            console.log(inc_data.HWC_WSID + " :: CROP CASE inserted :" + JSON.stringify(crop_result.affectedRows));
                        // else
                ...
                con_mdb.query(hwc_property_insertQuery, inpd_data, function (error, property_result, fields) {
                    if (error) {
                        ...
                        if (property_result.affectesRows > 0)
                            console.log(inpd_data.HWC_WSID + " :: PROPERTY CASE inserted :" + JSON.stringify(property_result.affectedRows));
                        // else
                ...
                con_mdb.query(hwc_livestock_insertQuery, inl_data, function (error, livestock_result, fields) {
                    if (error) {
                        ...
                        if (livestock_result.affectedRows > 0)
                            console.log(inl_data.HWC_WSID + " :: LIVESTOCK CASE inserted:" + JSON.stringify(livestock_result.affectedRows));
                        // else
